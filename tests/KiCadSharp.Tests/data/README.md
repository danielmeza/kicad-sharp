...
| `derived-symbols.kicad_sym` | **kicad-cli 10.0.6 wrote it** | 3,734 bytes, `(version 20251024)`, `(generator "kicad_symbol_editor")`. The only symbol fixture with a derived symbol: `DERIVED` `(extends "BASE")`. `BASE` has sub-units `BASE_0_1` and `BASE_1_1`; `BASE_X` is there because its sub-unit `BASE_X_1_1` also starts with `BASE_`, so a rename of `BASE` that matched sub-units by prefix would take it too. Regenerate with `kicad-cli sym upgrade --force -o derived-symbols.kicad_sym <input>`, from an input holding those three symbols with their four mandatory properties, `BASE` drawing one rectangle and two pins, `BASE_X` one pin. |
...
| `kicad10-pcbnew.kicad_pro` | hand-written | The smallest project file KiCad accepts: `meta` only. The IPC harness copies it beside the board so KiCad runs a real project rather than a null one; MEASURED against master, `SetTextVariables` answers "failed to save project text variables" without it. |
| `kicad10-pcbnew.kicad_pcb` | **pcbnew 10.0.6 wrote it** | 5,986 bytes. The only board in this table any KiCad wrote: `(version 20260206)`, `(generator "pcbnew")`, no board-level net table, every net named — `(segment … (net "GND"))` — a zone the filler filled into one 16-point `filled_polygon`, and KiCad's own spelling of a track `arc`, a `dimension` and a `group`. Regenerate by loading a board in `pcbnew`, running `ZONE_FILLER`, and `SaveBoard`. |
