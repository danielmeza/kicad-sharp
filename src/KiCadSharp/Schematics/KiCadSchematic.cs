...
namespace KiCadSharp.Schematics
{
    ...
    public sealed class KiCadSchematic : KiCadNode
    {
        ...
        /// <summary>
        /// The drawing-frame text, or <see langword="null"/> when the sheet has no
        /// <c>(title_block ...)</c>.
        /// </summary>
        /// <remarks>
        /// Reading this does not add the form. A getter that created one would dirty a document
        /// nobody edited and put an empty <c>(title_block)</c> into the next save — which is exactly
        /// the kind of write this library exists to avoid. Use <see cref="RequireTitleBlock"/> to
        /// give a sheet its first title.
        /// </remarks>
        ...
        public KiCadNodeList<KiCadSymbol> RequireLibrarySymbols() => new(Require("lib_symbols"), "symbol", n => new KiCadSymbol(n));
